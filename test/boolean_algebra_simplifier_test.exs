defmodule BooleanAlgebraSimplifierTest do
  ...
  describe "basic expression simplification" do
    test "simplifies OR expressions" do
      assert BooleanAlgebra.simplify(AST.or_node(AST.var_node(:a), AST.const_node(false))) == AST.var_node(:a)
      assert BooleanAlgebra.simplify(AST.or_node(AST.var_node(:a), AST.const_node(true))) == AST.const_node(true)
      assert BooleanAlgebra.simplify(AST.or_node(AST.const_node(false), AST.const_node(false))) == AST.const_node(false)
      assert BooleanAlgebra.simplify(AST.or_node(AST.const_node(true), AST.const_node(false))) == AST.const_node(true)
    end

    test "simplifies AND expressions" do
      assert BooleanAlgebra.simplify(AST.and_node(AST.var_node(:a), AST.const_node(false))) == AST.const_node(false)
      assert BooleanAlgebra.simplify(AST.and_node(AST.var_node(:a), AST.const_node(true))) == AST.var_node(:a)
      assert BooleanAlgebra.simplify(AST.and_node(AST.const_node(true), AST.const_node(true))) == AST.const_node(true)
      assert BooleanAlgebra.simplify(AST.and_node(AST.const_node(false), AST.const_node(true))) == AST.const_node(false)
    end
    ...
    test "simplifies NOT expressions" do
      ...
      assert BooleanAlgebra.simplify(AST.not_node(AST.var_node(:a))) == AST.not_node(AST.var_node(:a))
    end

    test "simplifies XOR expressions" do
      assert BooleanAlgebra.simplify(AST.xor_node(AST.var_node(:a), AST.const_node(false))) == AST.var_node(:a)
      assert BooleanAlgebra.simplify(AST.xor_node(AST.var_node(:a), AST.const_node(true))) == AST.not_node(AST.var_node(:a))
      assert BooleanAlgebra.simplify(AST.xor_node(AST.const_node(false), AST.const_node(false))) == AST.const_node(false)
      assert BooleanAlgebra.simplify(AST.xor_node(AST.const_node(true), AST.const_node(true))) == AST.const_node(false)
      assert BooleanAlgebra.simplify(AST.xor_node(AST.var_node(:a), AST.var_node(:a))) == AST.const_node(false)
    end
  end

  describe "complex expression simplification" do
    test "simplifies nested expressions" do
      complex_expr = AST.or_node(
